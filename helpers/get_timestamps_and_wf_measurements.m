function get_timestamps_and_wf_measurements(root_path, show_plots)
    ...
    for i=1:length(datafolders_names)
        [~, data_folders{end+1}, ~] = fileparts(datafolders_names{i});
        kilosort_folders{end+1} = fullfile(data_folders{i}, 'kilosort4');
    end
    ...
    for i = 1:numel(data_folders)
        ...
        cur_bin_path = fullfile(root_path, data_folders{i});
        cur_ks_path = fullfile(root_path, kilosort_folders{i});
        cur_save_path = fullfile(cur_ks_path, 'Spike times');
        if (exist(cur_save_path) == 0)
            mkdir(cur_save_path); 
        end
        % Create a folder called Waveforms info within saved directory, because
        % older versions of Phy crash if random .csv files are present
        cur_wfinfo_path = fullfile(cur_ks_path, 'Waveforms info');
        if (exist(cur_wfinfo_path) == 0)
            mkdir(cur_wfinfo_path); 
        end
        ...
        old_files = dir(strcat(cur_save_path, filesep, '*cluster*txt'));
        for i=1:numel(old_files)
            delete(fullfile(old_files(i).folder, old_files(i).name));
        end
        
        old_files = dir(fullfile(cur_wfinfo_path,  '*_waveforms.csv'));
        ...
        gwfparams.dataDir = cur_ks_path;    % KiloSort/Phy output folder 
        gwfparams.ops = ops;
        ...
        try
            ...
            if strcmp(ME.identifier, 'MATLAB:needMoreRhsOutputs') % this error might happen when file has been moved
                % try to find the fclean file name within the ks path
                split_fclean_path = split(ops.fclean, filesep);
                fclean = split_fclean_path{end};
                gwfparams.fileName = fullfile(cur_ks_path, fclean); % .dat file containing the raw used for sorting
                % Update ops
                ops.fclean = fullfile(cur_ks_path, fclean);
            end
        end

        % Save new config
        save(fullfile(cur_bin_path, 'config.mat'), 'ops');
        ...
        try
            ...
            if strcmp(ME.identifier, 'MATLAB:load:couldNotReadFile')
                ...
                fprintf(strcat('\t', ME.identifier))  % file not found has no identifier?? C'mon MatLab...
        ...
        save(fullfile(cur_wfinfo_path, strcat(prename, '_waveform_averages.mat')), 'wf', '-v7.3');
    ...
    %         yline(0)        
            ...
            title(strcat('Cluster', num2str(cluster_phy_id)));
            ...
            duration_txt = strcat('PTP = ', num2str(ptp_duration, 2), ' ms');
            ratio_txt = strcat('Ratio = ', num2str(ptp_ratio, 2));
            inflection_txt = strcat('Inflection = ', num2str(repolarization_duration, 3), ' ms');
        ...
        set(gcf, 'Position', [0 0 screen_size(3)/2 screen_size(4)/2] ); %set to half screen size
        set(gcf,'PaperPositionMode','auto') %set paper pos for printing
        print(strcat(cur_wfinfo_path, filesep, ...
            prename, '_waveform_measurements'), '-dpdf', '-bestfit', '-painters');
        ...
        writetable(TT, fullfile(cur_wfinfo_path, strcat(prename, '_waveform_measurements.csv')));
