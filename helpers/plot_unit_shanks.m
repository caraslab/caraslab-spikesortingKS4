function plot_unit_shanks(root_path, show_plots, load_previous_gwfparams, ...
    ...
    for folder_idx = 1:numel(datafolders)
        ...
        fprintf('Plotting waveform on electrode shanks for %s\n', cur_bin_path)
        ...
        gwfparams.channelPositions = readNPY(fullfile(cur_ks_path, 'channel_positions.npy')); % Vector of cluster shanks

        good_cluster_idx = wf.unitIDs; % store their Phy IDs

        % Delete old plots in folder
        old_files = dir(strcat(cur_wfinfo_path, filesep, '*shankWaveforms*pdf'));
        ...
        for wf_idx=1:length(good_cluster_idx)
            ...
            axis off;
           print(strcat(cur_wfinfo_path, filesep, prename,...
               '_shankWaveforms', '_cluster', num2str(cluster_phy_id)), '-dpdf', '-bestfit', '-painters');
        end
        fprintf('\tDONE!\n\n')
    end
end
