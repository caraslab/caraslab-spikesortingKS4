function [wf, gwfparams] = try_load_previous_gwfparams(bin_path, only_good)    
    % Load waveforms if gwfparams cannot be found in folder
    ks_path = fullfile(bin_path, 'kilosort4');
    try
        extracted_wfs = load(fullfile(ks_path, 'extracted_wfs.mat'));
        wf = extracted_wfs.wf;
    ...
    if ~exist('wf', 'var') || ~exist('gwfparams', 'var')
        ...
        [wf, gwfparams] = get_waveforms_from_folder(bin_path, only_good);
        % Save gwfparams and wf for future use
        fprintf('Saving gwfparams and wf structs to mat file\n')
        save(fullfile(ks_path, 'extracted_wfs.mat'), 'gwfparams', 'wf', '-v7.3');
    end
end